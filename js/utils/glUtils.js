...
class glUtils {
    constructor(osdViewer, markerScaleFactor) {
        ...
        this._markerScaleFactor = markerScaleFactor ? markerScaleFactor : 1.0;
        ...
        const osd = this.viewer.element.getElementsByClassName(
            "openseadragon-canvas"
        )[0];

        this._canvas =
            this.viewer.element.getElementsByClassName("gl_canvas")[0];
        ...
        this._programs["markers"] = this._loadShaderProgram(
            gl,
            MARKERS_VS,
            MARKERS_FS
        );
        this._buffers["barcodeMarkers"] = this._createDummyMarkerBuffer(
            gl,
            this._numBarcodePoints
        );
        this._buffers["CPMarkers"] = this._createDummyMarkerBuffer(
            gl,
            this._numCPPoints
        );
        ...
        document
            .getElementById("ISS_globalmarkersize_text")
            .addEventListener("input", drawHandler);
        document
            .getElementById("ISS_markers")
            .addEventListener("change", function (event) {
                self.updateLUTTextures();
            });
        document
            .getElementById("ISS_markers")
            .addEventListener("change", drawHandler);
...
glUtils.prototype.loadMarkers = function () {
    ...
    const useColorFromMarker =
        markerUtils._uniqueColor && colorPropertyName in markerData[0];
    let hexColor = "#000000";
    ...
    for (let i = 0; i < numPoints; ++i) {
        ...
        positions[4 * i + 2] =
            this._barcodeToLUTIndex[markerData[i].letters] / 4095.0;
...
glUtils.prototype._updateBarcodeToLUTIndexDict = function (
    markerData,
    keyName
) {
    ...
    for (let i = 0, index = 0; i < numPoints; ++i) {
        ...
        if (!(barcode in barcodeToLUTIndex)) {
            ...
            barcodeToKey[barcode] =
                keyName == "letters"
                    ? barcode
                    : dataUtils.getKeyFromString(gene_name);
        }
    }
...
glUtils.prototype._updateColorLUTTexture = function (gl, texture) {
    const allMarkersCheckbox = document.getElementById(
        "AllMarkers-checkbox-ISS"
    );
...
glUtils.prototype._formatHex = function (color) {
    if (color.includes("rgb")) {
        ...
        const hex = (Number(r) * 65536 + Number(g) * 256 + Number(b)).toString(
            16
        );
...
glUtils.prototype._updateColorScaleTexture = function (gl, texture) {
    ...
    for (let i = 0; i < 256; ++i) {
        ...
        if (
            this._colorscaleName.includes("interpolate") &&
            ...
            const g = Math.sin((normalized + 0.0) * 3.141592);
            const b = Math.sin((normalized + 0.33) * 3.141592);
            const s = 1.0 - normalized; // For purplish tone at end of the range
            colors[4 * i + 0] =
                Math.max(0.0, Math.min(1.0, r * (1.0 - 0.5 * b * b) + s * s)) *
                255.99;
            colors[4 * i + 1] =
                Math.max(0.0, Math.min(1.0, g * (1.0 - r * r * b * b))) *
                255.99;
            colors[4 * i + 2] =
                Math.max(0.0, Math.min(1.0, b * (1.0 - 0.5 * r * r))) * 255.99;
            colors[4 * i + 3] = 255.0;
        }
    }
...
glUtils.prototype._updateColorbarCanvas = function (
    colorscaleName,
    colorscaleData,
    propertyName,
    propertyRange
    ...
    if (
        !this._showColorbar ||
        colorscaleName == "null" ||
        colorscaleName == "ownColorFromColumn"
    )
        return;
...
glUtils.prototype._createColorbarCanvas = function () {
    ...
    canvas.style =
        "position:relative; float:left; width:31%; left:68%; " +
...
glUtils.prototype.draw = function () {
    ...
    gl.uniform4fv(
        gl.getUniformLocation(program, "u_viewportRect"),
        this._viewportRect
    );
    gl.uniformMatrix2fv(
        gl.getUniformLocation(program, "u_viewportTransform"),
        false,
        viewportTransform
    );
    gl.uniform2fv(
        gl.getUniformLocation(program, "u_markerScalarRange"),
        this._markerScalarRange
    );
    gl.uniform1f(
        gl.getUniformLocation(program, "u_markerOpacity"),
        this._markerOpacity
    );
    ...
    gl.uniform1f(
        gl.getUniformLocation(program, "u_markerScale"),
        this._markerScale
    );
    gl.uniform1i(
        gl.getUniformLocation(program, "u_useColorFromMarker"),
        this._useColorFromMarker
    );
    ...
    gl.uniform1f(
        gl.getUniformLocation(program, "u_markerScale"),
        this._markerScale * 0.5
    );
    gl.uniform1i(
        gl.getUniformLocation(program, "u_useColorFromMarker"),
        this._colorscaleName.includes("ownColorFromColumn")
    );
    if (this._colorscaleName != "null") {
        // Only show markers when a colorscale is selected
...
glUtils.prototype.updateMarkerScale = function () {
    const globalMarkerSize = Number(
        document.getElementById("ISS_globalmarkersize_text").value
    );
    ...
    this._markerScale =
        this._markerScaleFactor *
        Math.max(0.01, Math.min(5.0, globalMarkerSize / 100.0));
};
...
glUtils.prototype.updateLUTTextures = function () {
    ...
    if (this._numBarcodePoints > 0) {
        // LUTs are currently only used for barcode data
